-- ============================================================================
-- CORRECT APPROVAL FUNCTION
-- Money: Parents pay kids (no balance check, just add to kid's earnings)
-- Merets: Kids earn based on work quality (not transferred from parent)
-- ============================================================================
...
BEGIN
    -- Validate quality rating (1-5 stars)
    ...
    IF v_submission_status NOT IN ('pending_approval', 'submitted') THEN
        RETURN json_build_object('success', false, 'error', 'Submission is not pending approval');
    END IF;
    
    -- Get commitment data (task_template_id and pay_cents)
    ...
    -- Get base merets from task template
    ...
    -- Calculate merets earned based on quality
    -- Quality multiplier: 5 stars = 1.5x, 4 stars = 1.25x, 3 stars = 1.0x, 2 stars = 0.75x, 1 star = 0.5x
    ...
    v_merets_earned := ROUND((COALESCE(v_task_base_merets, 0) * v_merets_multiplier)::NUMERIC, 2);
    
    -- Calculate total money payment
    v_total_money_cents := COALESCE(v_commitment_pay_cents, 0) + COALESCE(p_bonus_tip_cents, 0);
    
    -- Get current stats for average rating calculation
    ...
    -- Calculate new average rating (always 1-5, safe for NUMERIC(5,2))
    v_new_avg_rating := (
    ...
    -- Update kid profile: add money, add merets, update stats
    ...
    RETURN json_build_object(
        ...
        'money_earned_dollars', v_total_money_cents / 100.0,
        'merets_earned', v_merets_earned,
        'quality_multiplier', v_merets_multiplier
    );
