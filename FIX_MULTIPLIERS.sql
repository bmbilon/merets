-- Fix Merets multipliers to correct values
-- 5★ = 120%, 4★ = 100%, 3★ = 70%, 2★ = 40%, 1★ = 20%

-- Drop and recreate approve_submission function with correct multipliers
...
-- Also update the quality_multiplier_for_rating helper function if it exists
DROP FUNCTION IF EXISTS quality_multiplier_for_rating(integer);

CREATE OR REPLACE FUNCTION quality_multiplier_for_rating(rating INTEGER)
RETURNS NUMERIC
LANGUAGE plpgsql
AS $$
BEGIN
